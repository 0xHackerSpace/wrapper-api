...
Conforme [ADR 0022](decisions/0022-agent-registration.md), o `ai-worker` também mantém **agents**: uma configuração nomeada e reutilizável (`system_prompt` + `model`/`temperature`/`max_tokens`/`top_p`) persistida em D1 dedicado (`dev-agents`, binding `AGENTS_DB`), separado de `dev-chat`. Desde a [ADR 0023](decisions/0023-agent-tool-calling.md), um agent também pode declarar `tools` (array de nomes de tools habilitadas, validado contra um catálogo fixo em código, `400` para nome desconhecido) e `max_tool_iterations` (teto de ciclos modelo→tool→modelo, default 5):
